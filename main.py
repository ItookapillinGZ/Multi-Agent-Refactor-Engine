...
class MessageBus:
    ...
    def read_inbox(self, name: str) -> list:
        ...
        msgs = [json.loads(l) for l in path.read_text(encoding="utf-8", errors="replace").strip().splitlines() if l]
        path.write_text("")
        return msgs
...
def sanitize_messages(messages):
    ...
    for m in messages:
        # 确保我们处理的是副本，避免干扰原始数据
        # 如果 m 是 Anthropic 的 Message 对象，转换为字典处理
        msg_dict = m if isinstance(m, dict) else m.model_dump() if hasattr(m, 'model_dump') else vars(m)

        if "content" in msg_dict and isinstance(msg_dict["content"], list):
            for i, block in enumerate(msg_dict["content"]):
                # 检查 block 类型（兼容对象和字典）
                b_type = block.get("type") if isinstance(block, dict) else getattr(block, "type", None)

                if b_type == "tool_use":
                    # 获取当前的 input
                    curr_input = block.get("input") if isinstance(block, dict) else getattr(block, "input", None)

                    # 关键修复：如果 input 满足以下条件，强制设为 {}
                    # 1. 是列表 []
                    # 2. 是 None
                    # 3. 根本不存在
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        # LLM call
        sanitized_history = sanitize_messages(messages)
        response = client.messages.create(
            model=MODEL, system=SYSTEM, messages=sanitized_history,
            tools=TOOLS, max_tokens=8000,
        )
